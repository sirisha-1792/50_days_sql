...
--1. dept avg salary (show each employee with avg salary of their dept)
select ce.emp_id, ce.dept_id, cs.salary, 
avg(cs.salary) over (partition by ce.dept_id) as dept_avg_salary
from cleaned_employees ce
join cleaned_salaries cs
on ce.emp_id = cs.emp_id;

--2. Total salary of each dept
select ce.emp_id, ce.dept_id, cs.salary, 
...
--3. avg performance of each dept (emp_id, dept_id)
